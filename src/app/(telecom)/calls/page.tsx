...
/* ---------------- helpers ---------------- */
// Replace your extractPhones(...) with this:
// Robust, multiline, format-agnostic parser
// === REPLACE THE WHOLE FUNCTION WITH THIS ===
function extractPhones(text, defaultCountry = "US") {
  // Split by lines first so we never miss multiline input
  const cleaned = new Set();

  const tokenRe = /[+\d][+\d()\[\]\-.\s]{6,}\d/g; // <- hyphen is safe here
  for (const line of text.split(/\r?\n/)) {
    const tokens = line.match(tokenRe) || [];
    for (let raw of tokens) {
      // drop trailing extensions (x123 / ext. 123)
      raw = raw.replace(/(ext\.?|x)\s*\d+$/i, "").trim();

      // keep only + and digits
      let d = raw.replace(/[^+\d]/g, "");

      // if '+' appears not at start, remove all '+'
      ...
      if (d.startsWith("+")) {
        ...
        d = digits; // fall through to NANP handling
      }

      // NANP (US/CA): 11 digits starting with 1 → drop the 1
      if (/^1\d{10}$/.test(d)) d = d.slice(1);

      // 10 digits → assume NANP and prefix +1
      if (/^\d{10}$/.test(d)) {
        cleaned.add("+1" + d);
        continue;
      }

      // 11–15 digits (no +) → best-effort international
      if (/^\d{11,15}$/.test(d)) {
        cleaned.add("+" + d);
        continue;
      }
              ...
              <div>
                ...
                <div className="relative mt-1 select-none">
                  <div className="mb-1 flex items-center justify-between text-xs text-zinc-500">
                    <span>1</span>
                    <span className="font-semibold text-zinc-700 dark:text-zinc-200">{cps} calls/sec</span>
                    <span>30</span>
                  </div>

                  <div className="relative h-3 w-full rounded-full bg-zinc-200/50 dark:bg-zinc-800">
                    {/* progress fill */}
                    <div
                      className="absolute inset-y-0 left-0 rounded-full bg-emerald-400/70"
                      style={{ width: `${((cps - 1) / (30 - 1)) * 100}%` }}
                    />
                    {/* native range overlay (for accessibility) */}
                    <input
                      type="range"
                      min={1}
                      max={30}
                    ...
                    <div
                      ...
                      style={{ left: `calc(${((cps - 1) / (30 - 1)) * 100}% - 14px)` }}
                    >
                  ...
                  <div className="mt-2 flex flex-wrap gap-1 text-xs">
                    {[1, 3, 5, 10, 20, 30].map((n) => (
                      <button
                        key={n}
                        onClick={() => setCps(n)}
                        className={`rounded-full px-2.5 py-1 ring-1 ring-inset ${cps === n
                            ? "bg-emerald-500/20 text-emerald-600 ring-emerald-400/40"
